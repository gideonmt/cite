...
int cite_export(const std::string& filename, const std::string& style, const std::string& output_file) {
    ...
    if (!output_file.empty()) {
        if (output_file.size() > 5 &&
            ...
            if (!outfile) { std::cerr << "Cannot open " << output_file << "\n"; return 3; }
            out = &outfile;
    ...
    auto print_html = [&](const std::string& text) {
        *out << text << "<br/>\n";
    };
